...
BASE_URL = "https://www.futnavarra.es"

START_URL = (
    "https://www.futnavarra.es/pnfg/NNws_LstNews"
    "?cod_primaria=3000217&cod_secundaria="
)
...
# La propia web indica "Página 1/25", así que recorremos
# exactamente esas 25 páginas.
TOTAL_PAGINAS = 25
...
def extraer_noticias(html):
    """Extrae las noticias de una página de FNF."""
    ...
    for bloque in soup.select("td.td_not"):
        ...
        # -------------------------------------------------
        # Fecha (viene junto al autor en el mismo <span>,
        # ej: "21/07/2026   Autor: Comunicación FNF")
        # -------------------------------------------------
...
def scrape():
    """
    Obtiene noticias de fútbol playa de la Federación Navarra
    de Fútbol (FNF).

    La web mezcla todas las categorías en el listado, así que
    hace falta filtrar por título o resumen. Se recorren las
    25 páginas del listado (paginación NPcd_Page).
    """

    print(f"FNF: procesando {START_URL}")
    ...
    for pagina in range(1, TOTAL_PAGINAS + 1):
        ...
        print(f"FNF: procesando página {pagina}: {url}")
        ...
        try:
            ...
            print(f"FNF: error descargando página {pagina}: {e}")
            continue

        except Exception as e:

            print(f"FNF: error procesando página {pagina}: {e}")
            continue

        if not noticias:
            print(f"FNF: no se encontraron noticias en página {pagina}")
            continue
        ...
        for noticia in noticias:
            ...
            print(f"FNF: fútbol playa -> {noticia['title']}")

        print(
            f"FNF: {encontradas_pagina} noticias de "
            f"fútbol playa en página {pagina}"
        )

    # -----------------------------------------------------
    # Ordenar las noticias de FNF
    # -----------------------------------------------------
    ...
    print(
        f"FNF: {len(noticias_futbol_playa)} noticias de "
...
if __name__ == "__main__":
    ...
    print()
    print("=== NOTICIAS FÚTBOL PLAYA (FNF) ===")
