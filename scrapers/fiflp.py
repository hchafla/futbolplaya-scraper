...
BASE_URL = "https://www.fiflp.com"

# cod_secundaria=3002061 ya apunta a la subcategoría
# "Noticias sobre la modalidad de Fútbol Playa", así que no
# hace falta filtrar por título, igual que RFAF/FFIB.
START_URL = (
    "https://www.fiflp.com/pnfg/NNws_LstNews"
    "?cod_primaria=3001834&cod_secundaria=3002061"
)
...
# La web dice "Página 1/1" (10 registros) ahora mismo, pero
# dejamos margen de seguridad por si crece con el tiempo.
MAX_PAGINAS = 6
...
def construir_url_pagina(pagina):
    ...
    return (
        ...
        f"?cod_primaria=3001834"
        f"&buscar="
        f"&cod_secundaria=3002061"
...
def extraer_noticias(html):
    """Extrae las noticias de una página de FIFLP."""
...
def scrape():
    ...
    Interinsular de Fútbol de Las Palmas (FIFLP).

    La subcategoría ya viene pre-filtrada por la propia web
    (cod_secundaria=3002061 = "Fútbol Playa"), así que no
    hace falta filtrar por título, igual que en rfaf.py /
    ffib.py.
    """

    print(f"FIFLP: procesando {START_URL}")
    ...
    with requests.Session() as sesion:
        ...
        for pagina in range(1, MAX_PAGINAS + 1):
            ...
            print(f"FIFLP: procesando página {pagina}: {url}")
            ...
            try:
                ...
                print(f"FIFLP: error descargando página {pagina}: {e}")
                break

            except Exception as e:

                print(f"FIFLP: error procesando página {pagina}: {e}")
                break
            ...
            print(f"FIFLP: {nuevas} noticias nuevas en página {pagina}")
    ...
    print(f"FIFLP: {len(noticias)} noticias de fútbol playa encontradas en total")
...
if __name__ == "__main__":
    ...
    print()
    print("=== NOTICIAS FÚTBOL PLAYA (FIFLP) ===")
