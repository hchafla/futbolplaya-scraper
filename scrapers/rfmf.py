...
BASE_URL = "https://www.rfmf.es"

START_URL = (
    "https://www.rfmf.es/pnfg/NNws_LstNews"
    "?cod_primaria=1000057&cod_secundaria="
)
...
# La web dice "Página 1/40" (592 registros), pero limitamos el
# recorrido como medida de seguridad (mismo motivo que en
# RFFPA: pasado cierto punto la paginación de este CMS puede
# dejar de responder correctamente).
MAX_PAGINAS = 20
...
def extraer_noticias(html):
    """Extrae las noticias de una página de RFMF."""
...
def scrape():
    ...
    Melillense de Fútbol (RFMF).

    La web mezcla todas las categorías en "Noticias Generales",
    así que hace falta filtrar por título o resumen. Se
    recorren como máximo MAX_PAGINAS páginas, manteniendo una
    sesión (cookies) entre peticiones.
    """

    print(f"RFMF: procesando {START_URL}")
    ...
    with requests.Session() as sesion:
        ...
        for pagina in range(1, MAX_PAGINAS + 1):
            ...
            print(f"RFMF: procesando página {pagina}: {url}")
            ...
            try:
                ...
                print(f"RFMF: error descargando página {pagina}: {e}")
                continue

            except Exception as e:

                print(f"RFMF: error procesando página {pagina}: {e}")
                continue

            if not noticias:
                print(f"RFMF: no se encontraron noticias en página {pagina}")
                continue

            print(f"RFMF: {len(noticias)} noticias en bruto en página {pagina}")
            ...
            for noticia in noticias:
                ...
                print(f"RFMF: fútbol playa -> {noticia['title']}")

            print(
                f"RFMF: {encontradas_pagina} noticias de "
                f"fútbol playa en página {pagina}"
            )

    # -----------------------------------------------------
    # Ordenar las noticias de RFMF
    # -----------------------------------------------------
    ...
    print(
        f"RFMF: {len(noticias_futbol_playa)} noticias de "
...
if __name__ == "__main__":
    ...
    print()
    print("=== NOTICIAS FÚTBOL PLAYA (RFMF) ===")
